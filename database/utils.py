...
async def add_payments(session: AsyncSession, mission: Mission, amount: int, count_payments_in_month: int):
    ...
    for i in range(0, mission.period_payments):
        for j in range(0, count_payments_in_month):
            ...
            if ((dd <= mission.date_created.day) & (i==0)): #если рассчитанный день платежа меньше или равен дню создания цели для первого месяца, переносим этот график на последний месяц
                d = date_created_begin_month + relativedelta(months=mission.period_payments) + relativedelta(days=(dd - 1))
            else:
    ...
    payments[mission.period_payments*count_payments_in_month-1].amount = amount + mission.total_amount - total_amount_cnt
