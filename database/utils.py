...
async def add_mission(session: AsyncSession, user_id: int, goal: str,
                      ...
                      period_payments: int) -> Optional[Mission]:
    mission = Mission(user_id=user_id, goal=goal, total_amount=total_amount,
                      income=income, income_frequency=income_frequency,
                      period_payments=period_payments)
    session.add(mission)
    await session.commit()
    logger.info(f'Создали миссию с {mission.id} для пользователя {mission.user_id}')
    return mission
