...
def add_mission (tg_user_id: int, username: str, goal: str,
                 total_amount: int, income: int, income_frequency: int,
                 period_payments:int, amount: int, count_payments_in_month: int):
    engine = create_engine('sqlite:///bot.db')
    session = Session(bind=engine)
    # проверяем наличие пользователя, если такого нет, добавляем
    user = session.query(User).filter(User.tg_user_id == tg_user_id).first()
    if (user == None):
    ...
    for i in range(0, period_payments):
        for j in range(0, count_payments_in_month):
            dd = j*ddays + (1 if (j==0) else 0)
            if ((dd <= mission.date_created.day) & (i==0)): #если рассчитанный день платежа меньше или равен дню создания цели для первого месяца, переносим этот график на последний месяц
                d = date_created_begin_month + relativedelta(months=period_payments) + relativedelta(days=(dd - 1))
            else:
    ...
    payments[period_payments*count_payments_in_month-1].amount = amount + total_amount - total_amount_cnt
